    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName('duration')
                .setDescription(
                    'Duration for temporary ban (e.g., "2d1h30m40s")'
                )
                .setRequired(false)
        ),
    ...
    async execute(interaction) {
        ...
        if (duration) {
            if (!durationRegex.test(duration)) {
                return interaction.reply({
                    content:
                        'Invalid duration format! Use something like `1d2h30m40s`.',
                    ephemeral: true,
                })
            }
...
function parseDuration(duration) {
    const days =
        (duration.match(/(\d+)d/)
            ? parseInt(duration.match(/(\d+)d/)[1], 10)
            : 0) *
        24 *
        60 *
        60 *
    ...
    const hours =
        (duration.match(/(\d+)h/)
            ? parseInt(duration.match(/(\d+)h/)[1], 10)
            : 0) *
        60 *
        60 *
        1000
    const minutes =
        (duration.match(/(\d+)m/)
            ? parseInt(duration.match(/(\d+)m/)[1], 10)
            : 0) *
        60 *
        1000
    const seconds =
        (duration.match(/(\d+)s/)
            ? parseInt(duration.match(/(\d+)s/)[1], 10)
            : 0) * 1000
