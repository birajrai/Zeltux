    ...
    static async formatEmbed(embed, channel) {
        ...
        if (embed.author) {
            ...
            if (embed.author.iconURL)
                embedText += ` (Icon: ${embed.author.iconURL})`
            embedText += ']\n'
        }

        if (embed.title) {
            const formattedTitle = await this.formatContent(
                embed.title,
                channel
            )
            embedText += embed.url
        ...
        if (embed.fields?.length) {
            ...
            for (const field of embed.fields) {
                const formattedName = await this.formatContent(
                    field.name,
                    channel
                )
                const formattedValue = await this.formatContent(
                    field.value,
                    channel
                )
        ...
        if (embed.footer) {
            ...
            if (embed.footer.iconURL)
                embedText += ` (Icon: ${embed.footer.iconURL})`
            embedText += ']\n'
        }
